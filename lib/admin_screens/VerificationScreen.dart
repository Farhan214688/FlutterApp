...
class _VerificationScreenState extends State<VerificationScreen>
  ...
  Future<void> _loadAllData() async {
    ...
    try {
      ...
      bool hasFirestoreData = _verifications.any((v) => !v.isLocalData) ||
          _offerApplications.isNotEmpty;

      if (!hasFirestoreData) {
        // If no Firestore data was loaded, show permission message
  ...
  Future<void> _loadVerifications() async {
    ...
    try {
      ...
      try {
        print('Debug - Loading verifications from Firestore');
        final QuerySnapshot snapshot = await _firestore
            .collection('professional_verifications')
            .orderBy('submittedDate', descending: true)
            .get();

        print('Debug - Found ${snapshot.docs.length} verifications in Firestore');

        final List<ProfessionalVerification> firestoreVerifications = snapshot
            .docs
            .map((doc) {
              print('Debug - Processing verification: ${doc.id}');
              return ProfessionalVerification.fromFirestore(doc);
            })
            .toList();
        ...
        setState(() {
          ...
          print('Debug - Total verifications after loading: ${_verifications.length}');
          print('Debug - Firestore verifications: ${firestoreVerifications.length}');
          print('Debug - Local verifications: ${localVerifications.length}');
        });
      } catch (e) {
        print('Debug - Error loading from Firestore: $e');
        // If Firestore access fails, try to load from local storage
        if (e.toString().contains('permission-denied')) {
          await _loadVerificationsFromLocal();
        } else {
          throw e;
        }
      }
    } catch (e) {
      print('Debug - Error in _loadVerifications: $e');
      setState(() {
  ...
  Future<void> _loadPendingVerifications() async {
    ...
    try {
      print('Debug - Starting to load pending verifications from Firebase');
      
      // First try to load from Firestore
      try {
        // Try loading without ordering first (this doesn't require a composite index)
        final QuerySnapshot regularVerificationsSnapshot = await _firestore
            .collection('professional_verifications')
            .where('status', isEqualTo: 'pending')
            .get();

        final QuerySnapshot offerVerificationsSnapshot = await _firestore
            .collection('professional_applications')
            .where('status', isEqualTo: 'pending')
            .get();

        print('Debug - Found ${regularVerificationsSnapshot.docs.length} regular pending verifications');
        print('Debug - Found ${offerVerificationsSnapshot.docs.length} offer pending verifications');

        // Process regular verifications
        final List<ProfessionalVerification> regularVerifications = regularVerificationsSnapshot.docs
            .map((doc) {
              try {
                final data = doc.data() as Map<String, dynamic>;
                print('Debug - Processing regular verification ${doc.id}: ${data}');
                return ProfessionalVerification.fromFirestore(doc);
              } catch (e) {
                print('Debug - Error processing regular verification ${doc.id}: $e');
                return null;
              }
            })
            .where((v) => v != null)
            .cast<ProfessionalVerification>()
            .toList();

        // Process offer verifications
        final List<ProfessionalVerification> offerVerifications = offerVerificationsSnapshot.docs
            .map((doc) {
              try {
                final data = doc.data() as Map<String, dynamic>;
                print('Debug - Processing offer verification ${doc.id}: ${data}');
                return ProfessionalVerification.fromFirestore(doc);
    } catch (e) {
                print('Debug - Error processing offer verification ${doc.id}: $e');
                return null;
              }
            })
            .where((v) => v != null)
        ...
        // Combine both lists
        final List<ProfessionalVerification> allPendingVerifications = [
          ...regularVerifications,
          ...offerVerifications,
        ];

        // Sort in memory instead of in the query
        allPendingVerifications.sort((a, b) => 
          b.submittedDate.compareTo(a.submittedDate));

        print('Debug - Total pending verifications found: ${allPendingVerifications.length}');

        // Validate the data
        for (var verification in allPendingVerifications) {
          if (verification.status != 'pending') {
            print('Debug - Warning: Found non-pending verification in results: ${verification.id}');
          }
          if (verification.name.isEmpty || verification.phoneNumber.isEmpty) {
            print('Debug - Warning: Found verification with missing required fields: ${verification.id}');
          }
        }

        // Update state with the combined list
    setState(() {
          _pendingVerifications = allPendingVerifications;
          print('Debug - Updated pending verifications list with ${_pendingVerifications.length} items');
        });

        // Store in local storage as backup
        await _updateLocalStorageWithFirestoreData(allPendingVerifications);

      } catch (e) {
        print('Debug - Error loading from Firestore: $e');
        if (e.toString().contains('failed-precondition')) {
          // Show a more helpful message about the index
      setState(() {
            _errorMessage = 'Setting up database indexes. This may take a few minutes. Please try again in a moment.';
          });
          print('Debug - Index creation required. Showing user-friendly message.');
        } else if (e.toString().contains('permission-denied')) {
          print('Debug - Permission denied, falling back to local storage');
          await _loadVerificationsFromLocal();
        } else {
      setState(() {
            _errorMessage = 'Failed to load verifications. Please check your internet connection and try again.';
          });
          throw e;
        }
      }
    } catch (e) {
      print('Debug - Error in _loadPendingVerifications: $e');
      if (!mounted) return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(_errorMessage.isNotEmpty 
            ? _errorMessage 
            : 'Error loading pending verifications: ${e.toString()}'),
  ...
  // Modified method to update local storage
  Future<void> _updateLocalStorageWithFirestoreData(List<ProfessionalVerification> firestoreVerifications) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      
      // Convert verifications to JSON-serializable format
      final List<Map<String, dynamic>> verificationsList = firestoreVerifications
          .map((v) {
            final map = v.toMap();
            // Ensure all required fields are present
            map['id'] = v.id;
            map['status'] = v.status;
            map['submittedDate'] = v.submittedDate;
            map['name'] = v.name;
            map['phoneNumber'] = v.phoneNumber;
            map['serviceName'] = v.serviceName;
            return map;
            })
            .toList();
      ...
      await prefs.setString(
        ...
        jsonEncode(verificationsList)
      );
      
      print('Debug - Updated local storage with ${verificationsList.length} verifications');
    } catch (e) {
      print('Debug - Error updating local storage: $e');
    }
  }

  // Modified method to load from local storage
  Future<void> _loadVerificationsFromLocal() async {
    try {
        final prefs = await SharedPreferences.getInstance();
        ...
        final List<ProfessionalVerification> localVerifications = verificationsList
            .map((data) => ProfessionalVerification.fromLocalStorage(data))
            .where((v) => v.status == 'pending')  // Only get pending verifications
            .toList();

        print('Debug - Loaded ${localVerifications.length} pending verifications from local storage');

        setState(() {
          _pendingVerifications = localVerifications;
        });

        // Try to sync with Firestore in the background
        _syncWithFirestore(localVerifications);
      }
          } catch (e) {
      print('Debug - Error loading verifications from local storage: $e');
    }
  }

  // New method to sync local data with Firestore
  Future<void> _syncWithFirestore(List<ProfessionalVerification> localVerifications) async {
    try {
      // Get current Firestore data
      final QuerySnapshot firestoreSnapshot = await _firestore
          .collection('professional_verifications')
          .where('status', isEqualTo: 'pending')
          .get();

      final Set<String> firestoreIds = firestoreSnapshot.docs
          .map((doc) => doc.id)
          .toSet();

      // Upload any local verifications that don't exist in Firestore
      for (final verification in localVerifications) {
        if (!firestoreIds.contains(verification.id)) {
          try {
            await _firestore
                .collection('professional_verifications')
                .doc(verification.id)
            ...
            print('Debug - Synced local verification ${verification.id} to Firestore');
          } catch (e) {
            print('Debug - Error syncing verification ${verification.id} to Firestore: $e');
          }
        }
      }
      } catch (e) {
      print('Debug - Error syncing with Firestore: $e');
    }
  }
  ...
  Future<void> _loadOfferApplications() async {
    ...
    try {
      ...
      setState(() {
        _errorMessage =
            'Failed to load offer applications. Please try again. Error: ${e.toString()}';
      });
    } finally {
      setState(() {
        _isLoading = false;
                ...
                Row(
                  ...
                  children: [
                              ...
                              Row(
                                children: [
                                  IconButton(
                                    ...
                                    tooltip: 'Activate',
                                    onPressed: () => _updateProfessionalStatus(
                                        verification, 'active'),
                                  ),
                                  IconButton(
                                    icon: Icon(
                                      Icons.block,
                                      color: currentStatus == 'deactivated'
                                          ? Colors.red
                                          : Colors.grey,
                                    ),
                                    tooltip: 'Deactivate',
                    ...
                    IconButton(
                      ...
                      onPressed: () => _deleteVerification(verification),
                    ),
                  ],
                ),
                SizedBox(height: 8),
                    ...
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (verification.locationType?.isNotEmpty == true)
                            Text(
  ...
  Future<void> _updateProfessionalStatus(
    ...
    try {
      ...
      // If reactivating, reset commission and completed orders count
      if (newStatus == 'active') {
        updateData['commission'] = 0;
        updateData['completedOrdersCount'] = 0;
        updateData['deactivationReason'] = FieldValue.delete();
        updateData['deactivatedAt'] = FieldValue.delete();
      }

      batch.update(professionalRef, updateData);
  ...
  // Helper: Get Firestore stream for pending verifications
  Stream<List<ProfessionalVerification>> getPendingVerificationsStream() {
    return _firestore
      .collection('professional_verifications')
      .where('status', isEqualTo: 'pending')
      .orderBy('submittedDate', descending: true)
  ...
  Widget _buildPendingVerificationList() {
    return StreamBuilder<List<ProfessionalVerification>>(
      stream: getPendingVerificationsStream(),
      builder: (context, snapshot) {
                          ...
                          Text('Phone: \\${verification.phoneNumber}'),
                              ...
                              if (verification.address != null ||
                                ...
                                crossAxisAlignment: CrossAxisAlignment.start,
                                    ...
                                    Text('Type: \\${verification.locationType}'),
                                      if (verification.latitude != null &&
                                          verification.longitude != null)
                                    Text('Coordinates: \\${verification.latitude!.toStringAsFixed(6)}, \\${verification.longitude!.toStringAsFixed(6)}'),
                                    ],
                                  ),
                                ),
                                SizedBox(height: 16),
                              ...
                              Row(
                                ...
                                children: [
                                  if (verification.status == 'pending') ...[
                                    ...
                                    ElevatedButton(
                                      onPressed: () =>
                                      _approveLocalVerification(verification),
                                      style: ElevatedButton.styleFrom(
                                          backgroundColor: Colors.green),
                                      child: Text('Approve'),
                                    ),
  ...
  // Modified method to approve verification
  Future<void> _approveLocalVerification(ProfessionalVerification verification) async {
    try {
      ...
      // Create professional data for Firestore
      Map<String, dynamic> professionalData = {
        'id': verification.id,
        'name': verification.name,
        'phoneNumber': verification.phoneNumber,
        ...
        'approved_date': DateTime.now().toIso8601String(),
        'approved_by_admin': true,
        'frontIDUrl': imageUrls[0].isNotEmpty ? imageUrls[0] : verification.frontIDUrl,
        'backIDUrl': imageUrls[1].isNotEmpty ? imageUrls[1] : verification.backIDUrl,
        'selfieUrl': imageUrls[2].isNotEmpty ? imageUrls[2] : verification.selfieUrl,
      };
      ...
      if (verification.latitude != null && verification.longitude != null) {
        ...
        professionalData['coordinates'] = GeoPoint(verification.latitude!, verification.longitude!);
      }
      ...
      // Update in professionals collection
      final professionalRef = _firestore.collection('professionals').doc(verification.id);
      batch.set(professionalRef, professionalData, SetOptions(merge: true));

      // Update in professional_verifications collection
      final verificationRef = _firestore.collection('professional_verifications').doc(verification.id);
      ...
      final accountStatusRef = _firestore.collection('account_status').doc(verification.id);
      batch.set(accountStatusRef, {
          'userId': verification.id,
          'userType': 'professional',
          'isActive': true,
      ...
      await _updateLocalVerificationStatus(verification.id, 'approved', imageUrls);
      ...
      // Show success message
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Professional verification approved successfully')),
      );
      ...
      print('Debug - Error approving verification: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error approving verification: ${e.toString()}')),
      );
    }
  }

  // Modified method to reject verification
  Future<void> _rejectVerification(ProfessionalVerification verification) async {
    try {
      // Delete the verification instead of marking it as rejected
      await _deleteVerification(verification);

      // Show success message
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Professional verification deleted')),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error deleting verification: ${e.toString()}')),
      );
    }
  }
  ...
  Future<void> _showDeleteAllPendingConfirmationDialog() async {
    ...
    final result = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: Text('Delete All Pending Verifications'),
        content: Text(
            'Are you sure you want to delete all pending verification requests? This cannot be undone.'),
        actions: [
          TextButton(
  ...
  Future<void> _clearAllPendingVerifications() async {
    try {
      ...
      if (verificationsJson != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('All pending verifications have been deleted')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content:
                Text('Error deleting pending verifications: ${e.toString()}')),
      );
    }
  }
  ...
  Future<void> _deleteLocalVerification(
    ...
    try {
      ...
      if (verificationsJson != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Verification deleted successfully')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _deleteVerification(
    ...
    try {
      ...
      final bool confirm = await showDialog(
            context: context,
            builder: (context) => AlertDialog(
              title: Text('Delete Verification'),
              content: Text(
                  'Are you sure you want to delete this verification for ${verification.name}?'),
              actions: [
                TextButton(
                  onPressed: () => Navigator.pop(context, false),
                  child: Text('Cancel'),
      ...
      if (verification.isLocalData) {
        ...
        await _deleteLocalVerification(verification);
      } else {
        ...
        try {
          ...
          // If permission denied, just treat as local verification
          if (e.toString().contains('permission-denied')) {
            // Delete from local state
            setState(() {
              _verifications.removeWhere((v) => v.id == verification.id);
              _offerApplications.removeWhere((v) => v.id == verification.id);
              _pendingVerifications.removeWhere((v) => v.id == verification.id);
            });
          } else {
            throw e;
      ...
      String errorMessage = e.toString().contains('permission-denied')
          ? 'Permission denied. Application removed from local view only.'
          : 'Error deleting verification: ${e.toString()}';
  ...
  Future<void> _navigateToProfessional(String professionalId) async {
    try {
      ...
      try {
        ...
        if (e.toString().contains('permission-denied')) {
          // If permission denied, show friendly message
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text(
                    'Cannot access professionals due to permission restrictions')),
          );
        } else {
          throw e;
        }
  ...
  void _showPermissionDeniedMessage() {
    ...
    if (!_errorMessage.contains('permission')) {
      ...
      // Delay slightly to avoid multiple dialogs
      Future.delayed(Duration(milliseconds: 300), () {
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                'Using local mode only. Firestore access requires updated permissions.'),
            duration: Duration(seconds: 5),
            backgroundColor: Colors.orange,
          ),
        );
